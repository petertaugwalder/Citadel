...
derived input, never shown as a row and never required: **^TNX** (10s30s
one-liner), plus TLT's live effective duration.
...
A headline number is not an edge; a range that survives every leg is closer to
one. `--stress` splits the history into contiguous sub-periods, re-runs at 1 / 5 / 10 bps per side, and
...
- `tlt_scanner.py` — the scanner (single file)
...
- SCHD leg removed; the scanner is TLT-only again.
